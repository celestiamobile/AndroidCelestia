...
object RealPathUtils {
    fun getRealPath(
        ...
        return getRealPathFromURIAboveAPI19(context, fileUri, applicationId)
    }
                ...
                when {
                    isExternalStorageDocument(uri) -> {
                        ...
                        return if ("primary".equals(type, ignoreCase = true)) {
                            if (split.size > 1) {
                                getExternalStorageDirectory(context, applicationId) + "/" + split[1]
                            } else {
                                getExternalStorageDirectory(context, applicationId) + "/"
                            }
                    ...
                    !isTree && isDownloadsDocument(uri) -> {
                        ...
                        if (fileName != null) {
                            return getExternalStorageDirectory(context, applicationId) + "/Download/" + fileName
                        }
    ...
    private fun getExternalStorageDirectory(context: Context, applicationId: String): String {
        val rootPath = context.getExternalFilesDir(null)?.absolutePath ?: return ""
        val extra = "/Android/data/" + applicationId + File.separator + "files"
