...
class MainActivity : AppCompatActivity(R.layout.activity_main),
    ...
    private fun openURLOrScriptOrGreeting() {
        ...
        if (guide != null) {
            lifecycleScope.launch {
                showBottomSheetUtility(Utility.Web(URLHelper.buildInAppGuideURI(guide, lang), listOf("guide")))
            }
            cleanup()
            return
        }
        ...
        if (addon != null) {
            lifecycleScope.launch {
                try {
                    ...
                    showBottomSheetUtility(Utility.Addon(result))
        ...
        lifecycleScope.launch {
            try {
                ...
                showBottomSheetUtility(Utility.Web(URLHelper.buildInAppGuideURI(result.id, lang), listOf("guide")))
            } catch (ignored: Throwable) {}
        }
    }
    ...
    private fun onInfoActionSelected(action: InfoActionItem, item: Selection) {
        when (action) {
            ...
            is SubsystemActionItem -> {
                ...
                lifecycleScope.launch {
                    showBottomSheetUtility(Utility.SubsystemBrowser(item))
                }
            }
    ...
    private fun showInfo(selection: Selection) = lifecycleScope.launch {
        showBottomSheetUtility(Utility.Info(selection))
    }
    ...
    private fun openAddonDownload() {
        ...
        lifecycleScope.launch {
            showBottomSheetUtility(Utility.WebNavigation(builder.build()))
        }
    }
    ...
    private fun showGoTo() = lifecycleScope.launch {
        ...
        showBottomSheetUtility(Utility.GoTo(inputData, Selection()))
    }
    ...
    private suspend fun showBottomSheetUtility(utility: Utility) {
        hideOverlay(true)
        showBottomSheetUtilityDirect(utility)
    }
