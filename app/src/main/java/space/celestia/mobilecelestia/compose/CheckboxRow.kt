...
fun CheckboxRow(primaryText: String, modifier: Modifier = Modifier, secondaryText: String? = null, checked: Boolean, onCheckedChange: ((Boolean) -> Unit)) {
    Row(verticalAlignment = Alignment.CenterVertically, modifier = modifier.toggleable(checked, role = Role.Checkbox, onValueChange = onCheckedChange).defaultMinSize(minHeight = dimensionResource(id = if (secondaryText != null) R.dimen.list_item_two_line_min_height else R.dimen.list_item_one_line_min_height)).padding(
