...
fun WebView(
    ...
    client.onURLBlocked = onURLBlocked
    client.onPageCommitVisible = onPageCommitVisible
    ...
    AndroidView(
        factory = { context ->
            (factory?.invoke(context) ?: WebView(context)).apply {
                ...
                state.viewState?.let {
                    ...
                    navigator.resetNavigationEvents()
                }
        ...
        modifier = modifier,
        onRelease = {
            state.viewState = Bundle().apply { it.saveState(this) }
            onDispose(it)
        }
    )
}
...
open class AccompanistWebViewClient : WebViewClient() {
    ...
    open lateinit var onError: () -> Unit
        internal set
    open lateinit var onURLBlocked: (String) -> Unit
        internal set
    open lateinit var onPageCommitVisible: () -> Unit
        internal set
    ...
    override fun shouldOverrideUrlLoading(view: WebView?, url: String?): Boolean {
        ...
        if (url != null && content is WebContent.Url) {
            fun isURLAllowed(): Boolean {
                val uri = Uri.parse(url)
                val originalUri = Uri.parse(content.url)
                if (uri.host != originalUri.host || uri.path != originalUri.path) {
                    return false
                }
                for (key in content.matchingQueryKeys ?: listOf()) {
                    if (uri.getQueryParameter(key) != originalUri.getQueryParameter(key)) {
                        return false
                    }
                }
                return true
            ...
            if (content.filterURL && !isURLAllowed()) {
                onURLBlocked(url)
                return true
            }
        }
...
sealed class WebContent {
    data class Url(
        ...
        val matchingQueryKeys: List<String>? = null,
        val filterURL: Boolean = false
    ) : WebContent()
...
class WebViewNavigator(private val coroutineScope: CoroutineScope) {
    ...
    internal fun resetNavigationEvents() {
...
fun rememberWebViewState(
    ...
    matchingQueryKeys: List<String>? = null,
