...
@Composable
fun AboutScreen(paddingValues: PaddingValues, urlHandler: (String, Boolean) -> Unit, modifier: Modifier = Modifier) {
                ...
                when (item) {
                    is ActionItem -> {
                        TextRow(primaryText = item.title, primaryTextColor = MaterialTheme.colorScheme.primary, modifier = Modifier.clickable {
                            urlHandler(item.url, item.localizable)
                        })
                    }
                    is VersionItem -> {
...
@Composable
private fun ICPCFooter(urlHandler: (String, Boolean) -> Unit) {
    Box(contentAlignment = Alignment.Center, modifier = Modifier
        ...
        Text(text = "苏ICP备2023039249号-4A", color = colorResource(id = com.google.android.material.R.color.material_on_background_emphasis_medium), style = MaterialTheme.typography.bodySmall, modifier = Modifier.clickable {
            urlHandler("https://beian.miit.gov.cn", false)
        })
    }
}
...
private fun createAboutItems(context: Context): List<List<AboutItem>> {
    ...
    // Authors
    getInfo("CelestiaResources/AUTHORS", CelestiaString("Authors", "Authors for Celestia"), context)?.let {
        array.add(it)
    }

    // Translators
    getInfo("CelestiaResources/TRANSLATORS", CelestiaString("Translators", "Translators for Celestia"), context)?.let {
        array.add(it)
    }
